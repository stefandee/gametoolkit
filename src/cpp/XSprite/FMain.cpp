...
void TFormMain::MakeCombo()
{
    ...
    for (unsigned int i = 0; i < mCells.size(); i++) {
        cbCells->Items->Add(mCells[i]->GetName());
    }
}
//---------------------------------------------------------------------------

/* TODO : implement sprite saving using ofstream and unicode -> utf8 */
bool TFormMain::Save(std::wstring _fileName)
{
    ...
    for (unsigned int i = 0; i < mCells.size(); i++) {
        ...
        auto cellName = lCell->GetName();
        std::string cellNameUTF8 = UTF8Encode(cellName).c_str();

        outfile << cellNameUTF8.c_str() << '\t' << (int)lRect.left << '\t' << (int)lRect.top << '\t' << (int)lRect.right << '\t' << (int)lRect.bottom << '\t' << (int)lPoint.x << '\t' << (int)lPoint.y << '\n';
    }
...
bool TFormMain::Load(std::wstring _fileName)
{
    ...
    while (std::getline(infile, line)) {
        ...
        getline(iss, lName, '\t');
        iss >> lLeft >> lTop >> lRight >> lBottom >> lX >> lY;
        ...
        lCell->SetName(UTF8Decode(lName.c_str()));
        lCell->SetRect(lRect);
        lCell->SetPoint(lPoint);
...
void __fastcall TFormMain::btnEditClick(TObject* Sender)
{
    ...
    FormName->SetText(mCells[cbCells->ItemIndex]->GetName());

    if (FormName->ShowModal() == mrOk) {
        mCells[cbCells->ItemIndex]->SetName(FormName->GetText());
...
{
    LOG(INFO) << "Command line usage for XSprite tool:";
    LOG(INFO) << "xsprite inputbatchfile outputpath";
}
//---------------------------------------------------------------------------

void TFormMain::HandleCommandLine()
{
    LOG(INFO) << "XSprite Tool";
    LOG(INFO) << "---------------";
    ...
    UnicodeString outputPath = ParamStr(2);
    ...
    while (inFile.getline(line, 1024)) {
        ...
        try {
            ...
            LOG(ERROR) << "Cannot load bitmap -> " << line;
        ...
        // output file path
        UnicodeString outputFile = outputPath + ChangeFileExt(ExtractFileName(line), ".spr");

        // export
        if (!ExportAs4BitIndexedSprite(outputFile.c_str())) {
            LOG(ERROR) << "Cannot export -> " << outputFile.c_str();
        }
