...
#define PNGDIB_SRC_VERSION           20100
#define PNGDIB_SRC_VERSION_STRING   "2.1.0"
...
static uchar* uncompress_dib(LPBITMAPINFO lpbmi1, int infosize, void *lpbits1)
{
	...
	lpdib2=(LPBITMAPINFOHEADER)GlobalAlloc(GPTR,infosize);
	...
	lpbits2= (uchar*)GlobalAlloc(GPTR,bitssize);
	if(!lpbits2) { GlobalFree(lpdib2); return NULL; }
	...
	rvi=GetDIBits(hdc,hb,0,height, lpbits2,
...
static void my_error_fn(png_structp png_ptr, const char *err_msg)
{
	...
	//_snprintf(errinfop->errmsg,MAX_ERRMSGLEN,"%s",err_msg);
...
void png_read_wrapper(png_structp png_ptr, png_bytep data, png_size_t length)
{
	void* pTmp = png_get_io_ptr(png_ptr);
	if (!pTmp) {
		/// PANIC!!!
	}
	PP::Stream* pStream = (PP::Stream*)pTmp;
	pStream->read(data, length);
}

int read_png_to_dib(PNGD_P2DINFO *p2dp, PP::Stream& input_stream)
{
	png_structp png_ptr;
	png_infop info_ptr;
	jmp_buf jbuf;
...
//	FILE* fp = 0;
	...
	int has_alpha_trans;  // ==1 if image may have partial transparency
	...
	if(p2dp->structsize==48) { // a little backward compatibility
		imginfo=0;
	}
	else if(p2dp->structsize==sizeof(PNGD_P2DINFO)) {
		imginfo=1;
	}
	else {
...
//	png_init_io(png_ptr, fp);
	png_set_read_fn(png_ptr, &input_stream, &png_read_wrapper);
	...
	// Check to see if image has potentially any partial-transparency.
	// This is only used for an obscure gamma correction issue.
	has_alpha_trans=0;
	if(has_alpha_channel) {
		...
		if(has_trns) {
			...
			tmp_trans=NULL; tmp_num_trans=0;
			if(png_get_tRNS(png_ptr, info_ptr, &tmp_trans, &tmp_num_trans,
			   NULL))
			{
				for(i=0;i<tmp_num_trans;i++) {
					if(tmp_trans[i]>0 && tmp_trans[i]<255) {
						has_alpha_trans=1;
						break;
					}
				}
	...
	{      // process most CUSTOM background colors
		...
		// libpng will use bkgd.gray if bkgd.red==bkgd.green==bkgd.blue.
		...
		if(is_grayscale) {
			...
			if(bkgd.red!=bkgd.green || bkgd.red!=bkgd.blue) {
				...
				// Why does it work? I don't know. It does *not* add an
...
notrans:
	...
	if(p2dp->flags&PNGD_GAMMA_CORRECTION) {
		...
		if(!is_grayscale || png_bit_depth>8 || has_alpha_channel) {
			...
			// Hack for an apparent bug in libpng. If image may have
			// partially transparent pixels, need to force gamma always
			// to be on, even if screen_gamma*file_gamma==1.0.
			// This is bad because it uses direct structure access, but
			// I don't know any way around it without patching libpng.
			// Versions of libpng later than 1.2.1 **may** not need this.
			//  ( #define PNG_GAMMA 0x2000 )
			if(has_alpha_trans && (has_bkgd||(p2dp->flags&PNGD_USE_CUSTOM_BG))) {
	...
	lpdib = (uchar*)GlobalAlloc(GPTR,p2dp->dibsize);
	if(!lpdib) { rv=PNGD_E_NOMEM; goto abort; }
	p2dp->lpdib = (LPBITMAPINFOHEADER)lpdib;

	row_pointers=(uchar**)GlobalAlloc(GMEM_FIXED,height*sizeof(uchar*));
	...
	// expand 2bpp to 4bpp
	if(png_bit_depth==2) {
		tmprow = (uchar*)GlobalAlloc(GPTR, (width+3)/4 );
...
int write_dib_to_png(PNGD_D2PINFO *d2pp)
{
	...
	int iscompressed;
	int palsize;  // bytes in palette or bitfields
	...
	if(d2pp->lpbits) {
		...
		if(d2pp->dibsize && !iscompressed) {  // bounds check
			size= headersize+palsize+dib_bytesperrow*height;
			if(size>d2pp->dibsize) { rv=PNGD_E_BADBMP; goto abort; }
		}

		bits= &((uchar*)(d2pp->lpdib))[headersize+palsize];
	}

	dib_palette = (RGBQUAD*) ( &((uchar*)(d2pp->lpdib))[headersize] );
	...
	if(dib_bpp==16 || dib_bpp==32) {
		...
		newimage=(uchar*)GlobalAlloc(GMEM_FIXED,height*width*3);
		...
		for(y=0;y<height;y++) {
			for(x=0;x<width;x++) {
				...
				case 10:  case 11:  // 16-bit, format 555 (xRRRRRGG GGGBBBBB)
					...
					newimage[(y*width+x)*3+0]= (v & 0x0000001f)<<3; // blue
					newimage[(y*width+x)*3+1]= (v & 0x000003e0)>>2; // green
					newimage[(y*width+x)*3+2]= (v & 0x00007c00)>>7; // red
					break;
				case 12:            // 16-bit, format 565 (RRRRRGGG GGGBBBBB)
					v= bits[y*dib_bytesperrow+x*2+0] | (bits[y*dib_bytesperrow+x*2+1]<<8);
					newimage[(y*width+x)*3+0]= (v & 0x0000001f)<<3; // blue
					newimage[(y*width+x)*3+1]= (v & 0x000007e0)>>3; // green
					newimage[(y*width+x)*3+2]= (v & 0x0000f800)>>8; // red
					break;
		...
		newimage= uncompress_dib((LPBITMAPINFO)d2pp->lpdib, headersize+palsize, bits);
		if(!newimage) { rv=PNGD_E_NOMEM; goto abort; }
		for(i=0;i<height;i++) {
			if(topdown) 
