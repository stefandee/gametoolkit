...
class ZPointerlist : public ZBase
{
   public:
      ...
      // copy constructor
      ZPointerlist(const ZPointerlist& aPointerlist);

      // destructor
      virtual ~ZPointerlist();

      // assignment and concatination
      ZPointerlist& operator=(const ZPointerlist& aPointerlist);

      ZPointerlist& operator+=(const ZPointerlist& aPointerlist);

      // subscribing
      void* operator[](long aIndex) const;

      // list manipulation
      ZPointerlist& removeFromPos(long aIndex);

      ZPointerlist& addAtPos(long aIndex, void* aPointer);
      ...
      // find an element
      long find(const void* aPointer) const;
