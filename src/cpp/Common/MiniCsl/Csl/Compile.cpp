...
void ZCsl::getParamList(Function* aFunc, ZBoolean aComp)
{
   ...
   while (iSym != symRparen) {
      ...
      if (aComp) aFunc->iBlocks->addVar(parName2, iNulString, isConst);
      if (iSym != symComma) break;
      getSym();
   } // while
   if (isOptional) {
      if (iSym != symRbrack) throwExcept(msgMissRbrack);
      if (aComp) aFunc->iBlocks->addVar("argCount", One, zTrue);
      getSym();
   } // if
   if (aComp) {
         ...
         if (iSym==symBecomes) {
            ...
            if (iSym == symBegin) {
               ...
               bInst->add(cdPush)
                    ...
                    ->add(&body);
               aFunc->iBlocks->addVar(rawName, iNulString, varSym==symConst);
            } else {
               expr(aFunc, bInst);
               Instruction* blast = bInst->last();
               bInst->add(&varadr)
   ...
   int cntLevel)
{
   ...
   switch (iSym) {
      ...
      case symTry: {
         ...
         aFunc->iBlocks->addVar(ident+"[1]", iNulString, varSym==symConst);
...
ZCsl& ZCsl::loadScript(const ZString& aFileName, istream* aStr)
{
         ...
         if (iSym == symLparen) {
            ...
            for (;;) {
               ...
               if (iExtern) {
                  ...
                  iBlock->addVar(ident, iNulString, varSym==symConst, zFalse, zFalse, zTrue);
               } else {
                  ZString rawName(ident);
