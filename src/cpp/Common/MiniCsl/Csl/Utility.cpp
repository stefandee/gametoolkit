...
void ZCsl::trace(char aFlag, const char* aMessage)
{
   ...
   std::cerr << space << aFlag << aMessage << std::endl;
...
void ZCsl::exec(Function* aFunc)
{
   ...
   try {
      ...
      if (iCalls->iCppFunc.valid())
	  {
		  push(iCalls->iCppFunc.callback(this));
	  }
      else {
		  if (iCalls->iPsize) {
            interpret(iCalls);
...
void ZCsl::showLibs(long& depth)
{
   ...
   ZLibrary* lib = iLibs;
   while (lib && depth > 0) {
      const char* tp = lib->IsNative() ? "Native" : "API";
      log((lib->GetName()+" ("+tp+")").constBuffer());
      lib = lib->GetPrevLibrary();
      depth--;
   } // while
} // showIdents
